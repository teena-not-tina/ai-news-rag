...
- **Score threshold** — 분포 측정 후 컷오프 도입. 노이즈 청크 자동 필터링.
- **MMR retrieval** — `max_marginal_relevance_search`. 긴 기사의 중복 청크 회피.
- **Hybrid (BM25 + Vector)** — 키워드+의미 결합. (위 "하이브리드 검색" 항목과 통합)
- **Re-ranking** — top-20 검색 후 Cross-Encoder/Cohere로 top-5 재순위.
