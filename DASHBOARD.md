...
> **최종 업데이트**: 2026-05-25
> **상태**: ✅ **MVP 완료 (Task 01~12 전부 🟢)** — 33개 테스트 통과, ChatGPT vs Gemini 평가 완료
...
5요소 설명 → `get_llm(provider)` 추상화 → 시스템 프롬프트:
- **근거 부족 선언 강제** ("제공된 뉴스 데이터에서 관련 정보를 찾지 못했습니다" — 추측 금지)
- **Answer Span Grounding** (모든 문장에 `[출처 URL][기사 제목]` 인용 강제, 매핑 불가 문장은 답변에서 제외)
- **부정 의도 판별** (사용자가 "X 아닌 것"을 묻는지 LLM에 명시적으로 확인하도록 프롬프트)
- **Rank-only** (raw score는 LLM에 신호로 전달하되 절대 threshold X — provider 간 비교 불가)
- ADR 006에 **한계 명시 필수**: "Prompt-only grounding은 충분조건 아님 — citation laundering 가능. retrieval quality 자체를 대체할 수 없음. v2 MMR/Hybrid로 보완."
→ 3종 답변 검수.
- [ ] 5요소 설명 → [ ] 결정 컨펌 → [ ] 개발 → [ ] 검수 → [ ] 컨펌

### Task 09 — CLI (`main.py`)
`python main.py build` (인덱싱 실행) + `python main.py chat` (대화형 질의).
**Retrieval-hit logging 골격 포함** (jsonl append, DB X):
```json
{"timestamp": "...", "query": "...", "provider": "...", "k": 4,
 "retrieved_chunks": [{"title": "...", "source": "...", "score": ...}, ...],
 "answer": "...", "citations": [...]}
```
→ 운영 디버깅 + Task 11 평가 데이터 자동 축적. 위치: `data/logs/retrieval.jsonl` (gitignored).
E2E 데모 (질문 → 답변 시연).
- [ ] 개발 → [ ] 검수 → [ ] 컨펌
...
- Task 04: `test_chunking.py` (비공백 청크) ✅ 작성됨
- Task 06: `test_indexer.py` (FAISS save/load roundtrip) ✅ 작성됨
- Task 07: `test_search.py` (top-k 반환 / 메타데이터 보존) ✅ 작성됨
- Task 08: `test_answer.py` (답변에 URL 포함)
- **Task 10**: 누락 점검 + `test_e2e_smoke.py` (build → query → answer 1건)
  - **Retrieval assertion 필수**: `assert len(results) > 0` + `assert results[0].metadata["title"] == expected` — answer만 검증하면 LLM이 얼버무릴 수 있음.

- [ ] 누락 점검 → [ ] E2E 스모크 (retrieval assertion 포함) → [ ] 검수 → [ ] 컨펌

### ⭐ Task 11 — 평가 + Gemini 비교 (Golden Dataset v1 seed) → ADR `007-evaluation-metrics.md`
**Reframe (외부 리뷰 반영, 2026-05-23):** 단순 10문항 채점이 아니라 **"future-extensible Golden Dataset의 v1 seed"** 로 설계. v2에서 50→200 확장 가능한 뼈대.

1. **테스트 질문 10개 + Golden Dataset schema 적용** (정답있음 7 + 환각테스트 3) — 유저 컨펌 필수
   ```json
   {
     "query": "...",
     "expected_article_ids": [...],
     "acceptable_answer_keywords": [...],
     "failure_labels": ["negation" | "temporal" | "entity_ambiguity" | "comparison" | "out_of_domain" | ...]
   }
   ```
   → failure_labels가 핵심: v2에서 hybrid/MMR/query rewrite 도입 시 "어떤 개선이 어떤 failure를 줄였는지" 측정 가능.
2. `time.time()` + 토큰 로깅 추가 (Latency / Tokens / Cost)
3. **Retrieval logging jsonl** (Task 09 골격 활용 — 평가 실행 시 자동 축적)
4. OpenAI로 10문항 실행 → 결과 저장
5. Gemini로 갈아끼워 10문항 실행 → 결과 저장
6. **운영 metric 2종 측정** (외부 리뷰 추천):
   - **거부율**: "제공된 뉴스 데이터에서 관련 정보를 찾지 못했습니다" 비율 → hallucination safety signal
   - **Citation coverage ratio**: 답변 문장 수 대비 citation 연결된 문장 비율 → grounding quality proxy
7. 채점지: 정확도(1-5) / Context Noise(노이즈청크/4) / Citation 일치 / Latency / Tokens / 비용 + (step 6의 2 metric)
8. 결과 표 + 결론 작성 (Task 12 README에 사용)

**제외 (v2로):** Top-k entropy dashboard / 정식 Golden Dataset infra / Ragas 자동 평가 / Re-ranker benchmark
- [ ] 10문항 + schema 컨펌 → [ ] 자동 로깅 구현 → [ ] OpenAI 실행 → [ ] Gemini 실행 → [ ] 수동 채점 + 2 metric → [ ] 결론 → [ ] 컨펌
...
## 🔜 v2 Phase A — 멀티플랫폼 뉴스 파서 (Strategy Pattern)

> **목표:** 여러 플랫폼 뉴스를 한 폴더에 섞어 넣어도 자동 라우팅해 올바르게 정제.
> **상세 설계:** [`docs/decisions/v2-backlog.md`](docs/decisions/v2-backlog.md) "다중 뉴스 포털 파서" 항목.

| # | Task | 의존성 | 상태 | 비고 |
|---|---|---|---|---|
| A1 | `parser.py` → `src/parsers/aitimes.py` 분리 | — | ⬜ | **행동 불변 리팩토링** (기존 33 테스트 그대로 통과 = 안전망) |
| A2 | `src/parsers/__init__.py` dispatcher | A1 | ⬜ | `extract_domain` + `PARSERS` 라우팅. `parse_file` 인터페이스 유지(호출부 불변) |
| A3 | `parse_generic` fallback | A2 | ⬜ | 미등록 도메인 — frontmatter만 + 보수적 본문 정리 |
| A4 | 2번째 플랫폼 파서 (네이버 클리핑) | **샘플 여러 개** | ⬜ | 샘플 모아 포맷 도출 후 작성 (n=1 단정 금지) |
| A5 | `tests/test_parsers.py` — 도메인 라우팅 + 섞인 코퍼스 | A2~A4 | ⬜ | |
| A6 | 멀티플랫폼 인덱싱·검색 E2E 검증 | A5 | ⬜ | source 메타에 플랫폼 구분 |

**시작 가능 시점:** A1~A3은 **샘플 없어도 지금 가능** (순수 구조 리팩토링). A4부터 2번째 플랫폼 md 샘플 여러 개 필요.

### A4 진행 방식: 샘플 수집 → 포맷 분석 → 라우팅 전략 결정
1. **샘플 수집** — 유저가 네이버 클리핑 기사 여러 개 제공 (n=1 단정 금지)
2. **포맷 분석** — 여러 샘플 공통 패턴 도출 (frontmatter 필드/본문 구조/footer/이미지 캡션 등)
3. **라우팅 전략 결정** ⚠️ — 네이버 클리핑은 출처 URL이 다 `n.news.naver.com` (네이버 **애그리게이터**, 원 매체는 지디넷 등 다양. URL 경로 `/article/092/`의 `092`=매체코드):
   - 가능성 A: 네이버가 포맷 통일 → `n.news.naver.com` **파서 1개**로 매체 무관 처리
   - 가능성 B: 매체별 포맷 다름 → 매체코드(092 등) 서브 라우팅
   - → **어느 쪽인지 여러 샘플로 확인 후 결정**

> 참고: 첫 샘플 = 지디넷코리아("AI가 환자 정신과 진료…견습생 수준", 092/0002424015). 유저가 계속 수집 예정.
